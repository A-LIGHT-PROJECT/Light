...
#error "Your operating system is not availiable for Light, sorry!"
	...
	void wrapping(std::queue<std::string> &wrappers) {
		while (true) {
			safe_lock.lock();
			if (wrappers.empty()) {
				safe_lock.unlock();
	...
	void interpretation(char Byte) {
			...
			while (someChar = file.get()) {
				if (stringMode) {
					if (someChar == OP_STR_WRAP) {
						stringMode = false;
						translationMap.insert({NeedFlag, tempString});
						tempString.clear();
						NeedFlag = SE_NOFLAG;
						continue;
					}
		...
		if (SetterMode) {
			if (translationMap.contains(Byte)) {
				wrapper += " " + translationMap.at(Byte);
			} else
	...
	void interpret(Bytecode bytecode) {
		...
		std::thread InterpretThread(interpretation);
